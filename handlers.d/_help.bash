...
Available slash commands:
  $_BOLD/help$_NC || $_BOLD/?$_NC      Show this help

  $_BOLD/agents$_NC [--select|--list]
                   --select (default) picks via numbered menu; --list prints.
  $_BOLD/agent$_NC <agent>   Set an agent (case-insensitive prefix match)

  $_BOLD/models$_NC [--select|--list]
                   --select (default) picks via numbered menu; --list prints.
  $_BOLD/model$_NC [name]    Show or set the current model
  ...
  $_BOLD/maxtokens$_NC <n>   Set max output tokens (positive integer)

  $_BOLD/ask$_NC <text>      Send a LLM query to claude code agent
  $_BOLD/$_NC <text>         Shorthand for /ask <text>
  ...
  $_BOLD/history$_NC [n]     Show the last n slash invocations; default 20
  $_BOLD/status$_NC          Print bash-reusable env dump of current session state
                   (\`source\` the output to round-trip into a fresh session)
  $_BOLD/sessions$_NC        List Claude Code sessions for the current cwd
  ...
  $_BOLD/exit$_NC            Exit the slash-bash
SLASH_CMD_HELP
}
