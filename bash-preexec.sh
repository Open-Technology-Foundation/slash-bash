...
#
# NOTE: This is a BCS-compliant version; not the original
#
...
declare -x __bp_imported=${bash_preexec_imported}
...
# function.
declare -i __bp_last_ret_value=$?
declare -a BP_PIPESTATUS=("${PIPESTATUS[@]}")
declare -- __bp_last_argument_prev_command="$_"

declare -i __bp_inside_precmd=0 __bp_inside_preexec=0

# Initial PROMPT_COMMAND string that is removed from PROMPT_COMMAND post __bp_install
#shellcheck disable=SC2016
declare -- __bp_install_string='__bp_install "$_"'
...
__bp_require_not_readonly() {
  ...
  for var; do
    if ! ( unset "$var" 2>/dev/null ); then
      >&2 echo "bash-preexec requires write access to ${var}"
      return 1
    fi
  done
}
...
__bp_invoke_precmd_functions() {
  ...
  for precmd_function in "${precmd_functions[@]}"; do
    ...
    if type -t "$precmd_function" 1>/dev/null; then
      ...
      if ((precmd_function_ret_value)); then
...
__bp_invoke_preexec_functions() {
  ...
  local -i preexec_function_ret_value
  local -i preexec_ret_value=0
  for preexec_function in "${preexec_functions[@]:-}"; do
    ...
    if type -t "$preexec_function" 1>/dev/null; then
      ...
      if ((preexec_function_ret_value)); then
